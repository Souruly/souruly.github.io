...
function draw()
{
   ...
   if(gotData==true)
   {
      for(var i=0 ; i<data.length ; i++)
      {
         if(data[i].set==false)
         {
            var x = fillWordIntoGrid(data[i].word, floor(random(gridSize)) , floor(random(gridSize)) , floor(random(0,2)) , floor(random(0,2)), i);
            if(x==1)
            {
               data[i].set=true;
               done++;
      ...
      else
      {
         ...
         textSize(20);
         var refreshMessage = "Please consider refreshing the page if the counter goes above 10  --------->";
